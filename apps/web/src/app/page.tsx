      ...
      <header className="border-b border-zinc-800">
        <div className="mx-auto flex w-full max-w-6xl items-center justify-between px-6 py-5">
          <div>
            ...
            <h1 className="text-2xl font-semibold">Académie CI</h1>
            <p className="mt-1 text-sm text-zinc-400">{CLUB.location}</p>
          </div>
      ...
      <main className="mx-auto flex w-full max-w-6xl flex-1 flex-col gap-12 px-6 py-16">
        <section className="max-w-3xl space-y-6">
          ...
          <h2 className="text-4xl font-semibold leading-tight sm:text-5xl">
            La plateforme officielle de {CLUB.name}.
          </h2>
          <p className="text-lg leading-8 text-zinc-400">
            Administration, suivi sportif, paiements en FCFA (Wave), convocations
            et communication — pour dirigeants, coaches, parents et joueurs à{" "}
            {CLUB.city}.
          </p>
        </section>
          ...
          {modules.map((module) => (
            ...
            >
              ...
              <p className="mt-2 text-sm text-zinc-400">
                Module du cahier des charges SAFFLE FF Académie.
              </p>
            </div>
          ))}
        </section>
        ...
        <section className="rounded-2xl border border-zinc-800 bg-gradient-to-br from-emerald-500/10 to-zinc-900 p-8">
          <h3 className="text-xl font-medium">Contact club</h3>
          <p className="mt-3 max-w-2xl text-zinc-400">
            {CLUB.name} — {CLUB.tagline}, basé à {CLUB.location}.
          </p>
          <div className="mt-4 flex flex-wrap gap-4 text-sm">
            <a
              href={CLUB.phoneHref}
              className="rounded-full border border-zinc-700 px-4 py-2 transition hover:border-emerald-500/50"
            >
              {CLUB.phone}
            </a>
            <a
